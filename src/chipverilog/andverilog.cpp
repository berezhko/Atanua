...
#include "Vandverilog.h"
#include "verilated.h"

ANDVerilog::ANDVerilog(int aUS)
{
    set(0,0,4,2,NULL);
    ...
    //Verilated::commandArgs(1, {"ANDVerilog", 0});
    myand = new Vandverilog("And");
}

void ANDVerilog::render(int aChipId)
{
	if (gBlackBackground)
...
void ANDVerilog::update(float aTick) 
{
    ...
    myand->a = (mInputPinA.mNet->mState == NETSTATE_HIGH);
    myand->b = (mInputPinB.mNet->mState == NETSTATE_HIGH);

    myand->eval();

    mOutputPin.setState(myand->out ? PINSTATE_WRITE_HIGH : PINSTATE_WRITE_LOW);
}    

ANDVerilog::~ANDVerilog()
{
    myand->final();
    delete myand;
}
